...
st.markdown("""
    ...
    <div class="main-header">
        <h1>📊 P2P Lending: Advanced Credit Scoring</h1>
...
with col1:
    ...
    # Employment length: Map text to numeric values for the model
...
with col2:
    ...
    # Term: Map text to numeric values (e.g., " 36 months" -> 36)
...
if st.button("🚀 Run Credit Risk Analysis", use_container_width=True):
    try:
        # --- DATA PRE-PROCESSING (Crucial for the 'median' error) ---
        
        # 1. Clean 'term': Convert " 36 months" -> 36
        term_numeric = int(term_raw.replace(' months', '').strip())
        
        # 2. Clean 'emp_length': Convert to number (e.g., "< 1 year" -> 0, "10+ years" -> 10)
        if '< 1' in emp_length_raw:
            emp_numeric = 0
        elif '10+' in emp_length_raw:
            emp_numeric = 10
        else:
            # Extract digits (e.g., "2 years" -> 2)
            emp_numeric = int(''.join(filter(str.isdigit, emp_length_raw)))

        # 3. Construct the DataFrame with exactly 11 columns
        input_data = pd.DataFrame({
            'loan_amnt': [loan_amnt],
            'term': [term_numeric], # Must be numeric
            'int_rate': [int_rate],
            'installment': [installment],
            'grade': [grade],
            'emp_length': [emp_numeric], # Must be numeric
        ...
        # Final check if object is valid
        if not hasattr(model, 'predict'):
            st.error("The file is a NumPy array. Please re-save the model object in your notebook.")
        else:
            # Prediction
            ...
            if prediction == 1:
                st.error(f"**High Alert:** Model indicates a high likelihood of default ({probability:.1%}).")
            else:
